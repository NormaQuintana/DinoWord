...
# var ordenar_data_cafeteria: Dictionary = {}
...
func _display_new_vocabulario_question():
	...
	var vocabulario_data = GlobalData.data.cafeteria.vocabulario
	if vocabulario_data.is_empty():
		vocabulario_pregunta_label.text = "No hay preguntas de vocabulario disponibles para 'cafeteria'."
		return
	...
	# La respuesta correcta es siempre la opción "1" en tu JSON de vocabulario
...
func _display_new_ordenar_question():
	...
	var ordenar_data = GlobalData.data.cafeteria.ordenar
	if ordenar_data.is_empty():
		ordenar_pregunta_label.text = "No hay preguntas de ordenar disponibles para 'cafeteria'."
		return
	...
	# Recopila todas las palabras que forman la frase ordenada
	for i in range(1, 11): # Asumiendo que las palabras están en claves "1", "2", ... "10"
		var key_word = str(i)
...
func _display_new_frases_question():
	...
	var frases_data = GlobalData.data.cafeteria.frases
	if frases_data.is_empty():
		frases_pregunta_label.text = "No hay preguntas de frases disponibles para 'cafeteria'."
		return
	...
	# La respuesta correcta es siempre la opción "1" en tu JSON de frases
	current_frases_correct_answer_text = question_data["1"]

	var options_list = []
	options_list.append(question_data["1"])
	options_list.append(question_data["2"])
	# Asegúrate de añadir las opciones 3 y 4 solo si existen
...
func _on_voz_pressed() -> void:
	...
	panel.visible = true
	# Aquí ya estás usando GlobalData.data.cafeteria.vocabulario, lo cual es correcto.
	var vocabulario = GlobalData.data.cafeteria.vocabulario
	print(vocabulario)
	var id_aleatorio = vocabulario.keys().pick_random()
	var pregunta = vocabulario[id_aleatorio]
	print(pregunta)
	palabra_voz = pregunta["p"]
	#$Preguntas/voz/Label.text = palabra_voz
...
func _on_audio_pressed() -> void:
	...
	if not texto_a_reproducir.is_empty():
		# Imprime las voces disponibles para depuración (esto es opcional, puedes quitarlo si quieres).
