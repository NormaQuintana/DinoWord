...
# var ordenar_data_oficina: Dictionary = {}
...
func _display_new_vocabulario_question():
	...
	var vocabulario_data = GlobalData.data.oficina.vocabulario
	if vocabulario_data.is_empty():
		vocabulario_pregunta_label.text = "No hay preguntas de vocabulario disponibles para 'oficina'."
		return
...
func _display_new_ordenar_question():
	...
	var ordenar_data = GlobalData.data.oficina.ordenar
	if ordenar_data.is_empty():
		ordenar_pregunta_label.text = "No hay preguntas de ordenar disponibles para 'oficina'."
		return
...
func _display_new_frases_question():
	...
	var frases_data = GlobalData.data.oficina.frases
	if frases_data.is_empty():
		frases_pregunta_label.text = "No hay preguntas de frases disponibles para 'oficina'."
		return
...
func _on_voz_pressed() -> void:
	...
	panel.visible = true
	# Aquí ya estás usando GlobalData.data.oficina.vocabulario, lo cual es correcto.
	var vocabulario = GlobalData.data.oficina.vocabulario
	print(vocabulario)
